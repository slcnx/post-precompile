...
if [ -n $release ]; then
	if [ $VERSION -eq 1 ]; then
		echof "${TARGET_DIR}/$APP_DIR -> ${TARGET_DIR}/$appname" 1 0
...
if [ $BIN -eq 1 ]; then
	...
	if [ $find_bin_count -ge 1 ]; then
		...
		echo ':$PATH' >> /etc/profile.d/$appname.sh
		echof "${TARGET_DIR}/$appname 目录中没有bin, sbin目录，导出默认目录" 1 0
	fi
fi
...
if [ $SYSTEMD -eq 1 ]; then
	...
	PROG_PATH=$(find ${TARGET_DIR}/$appname/  -name $PROG -type f -perm 755)
	if [ -z "$PROG_PATH" ]; then
